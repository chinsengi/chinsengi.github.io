...
times a constant. It suffices to show that \$n\\alp\^n\\to 0\$ as \$n\\to \\inf\$, where \$0\<\\alp\<1\$. Simply note that \$n\\alp\^{n-1}\$ is the derivative of \$\\alp\^n\$ which goes to 0 as \$n\$ goes to infinity. So we can always
choose large enough \$n\$ such that any
finite difference of \$\\alp\^n\$ around
\$\\alp\$ is below any small \$\\eps\$, QED.
...
increasingly smaller. More specifically, they are related by \$\\lam:=rT = np\$.

Now here is the mathematical show: \$\$
\\bsp
\\lim\_{\\lam = np,\\ n\\to \\inf}P(k) &= \\lim\_{rT = np,\\ n\\to
